...
def plot_samples(ground_truth, produced_images, epoch):
    ...
    ax4.imshow(produced_images[0].reshape((14, 14)), cmap="gray")
    plt.title("Sampled")
    ax5 = fig.add_subplot(2, 3, 5)
    ax5.imshow(produced_images[1].reshape((14, 14)), cmap="gray")
    plt.title("Sampled")
    ax6 = fig.add_subplot(2, 3, 6)
    ax6.imshow(produced_images[2].reshape((14, 14)), cmap="gray")
    plt.title("Sampled")
...
def plot_rdf(ground_truth, means, variances, epoch):
    ...
    for comp in range(num_mix):
        ...
        # mean
        ax[1, comp].imshow(
            means[comp].reshape((14, 14)), cmap="gray", extent=(-56, 56, -56, 56)
        )
        ax[1, comp].set_title("Mean")

        # mean
        ax[2, comp].imshow(
            variances[comp].reshape((14, 14)), cmap="gray", extent=(-56, 56, -56, 56)
        )
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        sub_sampled_image = transform.resize(image[0], (14, 14))
...
train_loader = torch.utils.data.DataLoader(
    datasets.MNIST(
        "/home/taras/Documents/Datasets/",
        train=True,
        download=True,
        transform=transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize((0.0,), (1.0,)), Rescale()]
        ),
    ),
...
netw = MLP(1, dim * 2, 512).to(device)

aff_flow = [AffineConstantFlow(dim=dim) for i in range(7)]
coupling_flow = [CouplingLayer(dim, 256, i % 2 == 1) for i in range(7)]
maf_flow = [SlowMAF(dim=dim, parity=True) for _ in aff_flow]
norms = [ActNorm(dim=dim) for _ in coupling_flow]
flows = list(itertools.chain(*zip(aff_flow)))
